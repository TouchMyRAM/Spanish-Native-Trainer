...
void check_player_model() 
{
	...
	if (ENTITY::IS_ENTITY_DEAD(playerPed) || PLAYER::IS_PLAYER_BEING_ARRESTED(player, TRUE))
		...
		{
			set_status_text("volviendo a la normalidad");
			WAIT(1000);
...
bool process_teleport_menu()
{
	...
	std::string caption = "Teletransportarse";
	...
	static struct {
		...
		float z;
			...
			{ "MARCADOR" },
			{ "CASA DE MICHAEL", -852.4f, 160.0f, 65.6f },
			{ "CASA DE FRANKLIN", 7.9f, 548.1f, 175.5f },
			{ "TRAILER DE TREVOR", 1985.7f, 3812.2f, 32.2f },
			{ "ENTRADA AEROPUERTO", -1034.6f, -2733.6f, 13.8f },
			{ "CAMPO DE AEROPUERTO", -1336.0f, -3044.0f, 13.9f },
			{ "ELYSIAN ISLAND", 338.2f, -2715.9f, 38.5f },
			{ "JETSAM", 760.4f, -2943.2f, 5.8f },
			{ "CLUB DE STRIPTEASE", 127.4f, -1307.7f, 29.2f },
			{ "ELBURRO HEIGHTS", 1384.0f, -2057.1f, 52.0f },
			{ "LA NORIA", -1670.7f, -1125.0f, 13.0f },
			{ "CHUMASH", -3192.6f, 1100.0f, 20.2f },
			{ "PARQUE EÓLICO", 2354.0f, 1830.3f, 101.1f },
			{ "BASE MILITAR", -2047.4f, 3132.1f, 32.8f },
			{ "CAMPO DE AVIACIÓN DE MCKENZIE", 2121.7f, 4796.3f, 41.1f },
			{ "CAMPO DE AVIACIÓN DE DESIERTO", 1747.0f, 3273.7f, 41.1f },
	...
	while (true)
	{
		...
		if (bSelect)
		{
			...
			{			
				...
				{
					set_status_text("el marcador del mapa no se ha establecido");
				}
				...
				case 3:
					{
						...
						set_status_text("UTILIZANDO PIEL PREDETERMINADA");
					}
					break;
				// add cash
				case 4: 
		...
		if (bSelect)
		{
			...
			switch (activeLineIndexWeapon)
			{
				case 0:
					...
					set_status_text("DADAS TODAS LAS ARMAS");
					break;
				// switchable features
				default:
...
bool process_carspawn_menu()
{
	...
	while (true)
	{
		...
		do 
		{
			// draw menu
			char caption[32];
			sprintf_s(caption, "UN COCHE SPAWN   %d / %d", carspawnActiveLineIndex + 1, lineCount);
			draw_menu_line(caption, 350.0, 15.0, 18.0, 0.0, 5.0, false, true);
			for (int i = 0; i < itemCount; i++)
		...
		if (bSelect)
		{
			...
			if (STREAMING::IS_MODEL_IN_CDIMAGE(model) && STREAMING::IS_MODEL_A_VEHICLE(model))
			{
				...
				char statusText[32];
				sprintf_s(statusText, "%s GENERADO", modelName);
			...
			switch (activeLineIndexVeh)
			{
				...
				case 1:
					if (bPlayerExists) 
					{
						...
						{
							set_status_text("NO ESTAS EN UN VEHÍCULO");
						}
					}
					break;
				case 2:
...
void process_weather_menu()
{
	...
	std::string caption = "OPCIONES  METEOROLÓGICAS";
	...
	static struct {
		...
		{"VIENTO",	 	 &featureWeatherWind,	NULL},
		{"PERSISTENTE",  &featureWeatherPers,	NULL},
		{"MÁS SOL",	 NULL,					NULL},
		{"CLARO",		 NULL,					NULL},
		{"NUBLADO",		 NULL,					NULL},
		{"NIEBLA",		 NULL,					NULL},
		{"NIEBLA [2]",	 	 NULL,					NULL},
		{"NUBLADO [VERIFY]",	 NULL,					NULL},
		{"LLUVIA",		 NULL,					NULL},
		{"RAYO",		 NULL,					NULL},
		{"CIELOS DE COMPENSACIÓN",	 NULL,					NULL},
		{"NIEVE",		 NULL,					NULL},
		{"TORMENTA DE NIEVE",	 NULL,					NULL},
		{"NIEVE [2]",	 NULL,					NULL},
		{"NAVIDAD",		 NULL,					NULL}
	};
...
void process_main_menu()
{
	...
	std::string caption = "TRAINER ESPAÑOL (AB)";
